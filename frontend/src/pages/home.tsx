    ...
    <div>
      ...
      <div className="w-full [container-type:inline-size] [container-name:home-shell]">
        ...
        <div>
          ...
          <div className="relative flex flex-col min-h-[min(44rem,calc(100svh-2rem))] @md:min-h-[50rem] overflow-hidden isolate bg-[#05060a] [container-type:inline-size] [container-name:hero-shell]">
            {/* background art */}
            <div className="absolute inset-0 z-0">
              <div className="relative w-full h-full overflow-hidden">
                ...
                <img
                  ...
                  alt=""
                  className="relative z-0 w-full h-full object-cover object-[center_bottom] block [filter:saturate(1.05)_brightness(1.15)] min-h-[min(44rem,calc(100svh-2rem))] @md:min-h-[50rem]"
                />
            ...
            <div className="relative z-[2] flex-1 flex flex-col items-center justify-center gap-[1.05rem] @sm:gap-[1.1rem] @md:gap-[1.15rem] w-full @md:w-[min(100%,46rem)] self-center px-[1.2rem] py-[2rem] @sm:p-[2rem_1.35rem] @md:p-[2.5rem_2rem] @lg:p-[3rem_2.5rem] text-center transition-[width,padding] duration-[240ms] ease-[cubic-bezier(0.22,1,0.36,1)]">
              <p className="eyebrow">Noxian Netdecks</p>
              <h1 className="flex flex-col items-center gap-[0.05em] m-0 text-[clamp(4.65rem,17cqi,6.8rem)] @sm:text-[clamp(3.1rem,8.4vw,4.25rem)] @md:text-[4.75rem] @lg:text-[4.75rem] leading-[0.94] tracking-[-0.055em] transition-[font-size,transform] duration-[180ms]">
                <span className="text-text-primary text-[0.68em] @md:text-[0.7em]">The Complete</span>
              ...
              <p className="hidden @md:block max-w-[30rem] @lg:max-w-[26rem] @lg:text-[1.15rem] m-0 text-text-secondary text-base leading-[1.55]">Search cards, understand price trends.</p>
              <div
                ref={heroFormRef}
                className="mt-[0.3rem] w-full max-w-full @sm:w-[min(100%,34rem)] @md:w-[min(100%,40rem)] @lg:w-[min(100%,50rem)] transition-[width] duration-[180ms]"
              >
                <CardSearchInput
                  value={query}
                  onChange={setQuery}
            ...
            <div className="relative z-[4] px-[var(--space-shell-x)] pt-[1rem] pb-[1.5rem] @md:pb-[2rem]">
              <div className="grid gap-[0.85rem] [grid-template-columns:1fr] @md:grid-cols-2 @lg:grid-cols-3 [container-type:inline-size]">
                <TileFeature
        ...
        {/* content section */}
        <div className="grid gap-[0.85rem] @md:gap-4 @xl:gap-[1.1rem] pt-[0.85rem] @md:pt-4 px-[var(--space-shell-x)]">
          {[0, 1, 2, 3, 4].map((i) => (
            <div key={i} className="grid gap-[0.85rem] @md:gap-4">
              {/* promo grid */}
              <div className="grid gap-[0.85rem] [grid-template-columns:1fr] @sm:grid-cols-2 [container-type:inline-size]">
                <TilePromo
                  label="Tool"
